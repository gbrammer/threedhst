...
for file in $files; do 
	...
	if [ "$gz" == "" ]; then 
	    line=`cat $file | dfits - | fitsort TARGNAME  DATE-OBS        TIME-OBS   ${filter}  EXPTIME         PA_V3 RA_TARG DEC_TARG  POSTARG1 POSTARG2 |tail -1`;
    else
	    line=`gunzip -c $file |dfits - |fitsort TARGNAME  DATE-OBS        TIME-OBS   ${filter}  EXPTIME         PA_V3 RA_TARG DEC_TARG POSTARG1 POSTARG2 |tail -1`;
    fi
